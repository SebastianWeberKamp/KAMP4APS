/**
 */
package edu.kit.ipd.sdq.kaps.model.modificationmarks.tests;

import edu.kit.ipd.sdq.kaps.model.modificationmarks.ModifyPhysicalConnection;
import edu.kit.ipd.sdq.kaps.model.modificationmarks.modificationmarksFactory;

import junit.framework.TestCase;

import junit.textui.TestRunner;

/**
 * <!-- begin-user-doc -->
 * A test case for the model object '<em><b>Modify Physical Connection</b></em>'.
 * <!-- end-user-doc -->
 * @generated
 */
public class ModifyPhysicalConnectionTest extends TestCase {

	/**
	 * The fixture for this Modify Physical Connection test case.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @generated
	 */
	protected ModifyPhysicalConnection<?> fixture = null;

	/**
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @generated
	 */
	public static void main(String[] args) {
		TestRunner.run(ModifyPhysicalConnectionTest.class);
	}

	/**
	 * Constructs a new Modify Physical Connection test case with the given name.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @generated
	 */
	public ModifyPhysicalConnectionTest(String name) {
		super(name);
	}

	/**
	 * Sets the fixture for this Modify Physical Connection test case.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @generated
	 */
	protected void setFixture(ModifyPhysicalConnection<?> fixture) {
		this.fixture = fixture;
	}

	/**
	 * Returns the fixture for this Modify Physical Connection test case.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @generated
	 */
	protected ModifyPhysicalConnection<?> getFixture() {
		return fixture;
	}

	/**
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @see junit.framework.TestCase#setUp()
	 * @generated
	 */
	@Override
	protected void setUp() throws Exception {
		setFixture(modificationmarksFactory.eINSTANCE.createModifyPhysicalConnection());
	}

	/**
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @see junit.framework.TestCase#tearDown()
	 * @generated
	 */
	@Override
	protected void tearDown() throws Exception {
		setFixture(null);
	}

} //ModifyPhysicalConnectionTest
