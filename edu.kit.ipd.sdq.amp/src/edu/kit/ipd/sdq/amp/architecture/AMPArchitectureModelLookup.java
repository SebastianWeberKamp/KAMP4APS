package edu.kit.ipd.sdq.amp.architecture;
 ...
 */
public class AMPArchitectureModelLookup {
	...
	public static <T> Set<T> lookUpMarkedObjectsOfATypeInAnIterator(
		...
		while (objectsToSearch.hasNext()) {
			...
			if (object instanceof AbstractModification<?,?>) {
				...
				Object affectedElement = modification.getAffectedElement();
				boolean isSameClassAsObjectClass = objectClass.isAssignableFrom(modification.getAffectedElement().getClass());
				if (affectedElement != null && isSameClassAsObjectClass) {
