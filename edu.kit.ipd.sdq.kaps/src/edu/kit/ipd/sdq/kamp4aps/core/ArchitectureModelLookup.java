...
public class ArchitectureModelLookup {
	...
	public static List<AssemblyConnector> lookUpAssemblyConnectorsAttachedToProvidedRole(ArchitectureVersion version,
	...
	public static List<AssemblyContext> lookUpAssemblyContextsForRepositoryComponent(ArchitectureVersion version,
	...
	private static List<AssemblyContext> lookUpAssemblyContextsInCompositeComponents(ArchitectureVersion version,
			RepositoryComponent component) {
		List<AssemblyContext> assemblyContexts = new ArrayList<AssemblyContext>();
	 ...
	 */
	public static void lookUpInterfacesAndSignaturesWithParametersOfTypes(ArchitectureVersion version,
	...
	public static Map<Interface, Set<Signature>> lookUpInterfacesAndSignaturesWithSignatures(
			ArchitectureVersion version, Collection<Signature> signatures) {
	 ...
	 */
	public static void lookUpComponentsAndRolesWithInterfaces(ArchitectureVersion version,
	...
	public static Map<OperationProvidedRole, Set<Signature>> lookUpMarkedProvidedSignaturesForProvidedRoles(
			Collection<OperationProvidedRole> providedRoles, ArchitectureVersion version) {
		Map<OperationProvidedRole, Set<Signature>> results = new HashMap<OperationProvidedRole, Set<Signature>>();

		Set<OperationSignature> markedSignatures = AMPArchitectureModelLookup.lookUpMarkedObjectsOfAType(version,
		...
		for (OperationProvidedRole providedRole : providedRoles) {
			Set<Signature> providedSignatures = new HashSet<Signature>(
					providedRole.getProvidedInterface__OperationProvidedRole().getSignatures__OperationInterface());
	...
	public static ModifySignature lookUpProvidedSignatureMarkForRoleModification(Signature signature,
			ModifyProvidedRole modifyProvidedRole) {
	...
	public static Map<OperationRequiredRole, Set<Signature>> lookUpMarkedRequiredSignaturesForRequiredRoles(
			Collection<OperationRequiredRole> requiredRoles, ArchitectureVersion version) {
		Map<OperationRequiredRole, Set<Signature>> results = new HashMap<OperationRequiredRole, Set<Signature>>();

		Set<OperationSignature> markedSignatures = AMPArchitectureModelLookup.lookUpMarkedObjectsOfAType(version,
				OperationSignature.class);
		for (OperationRequiredRole requiredRole : requiredRoles) {
			Set<Signature> requiredSignatures = new HashSet<Signature>(
					requiredRole.getRequiredInterface__OperationRequiredRole().getSignatures__OperationInterface());
	...
	public static Set<Signature> lookUpMarkedRequiredSignaturesForRequiredRoleModification(
			ModifyRequiredRole modifyRequiredRole) {
		Set<Signature> requiredSignatures = new HashSet<Signature>();
	...
	public static Map<RequiredRole, List<RoleToRoleDependency>> lookUpInternalDependenciesWithRequiredRoles(
			ArchitectureVersion version, Collection<OperationRequiredRole> requiredRoles) {
	...
	public static ModifyProvidedRole lookUpMarkedProvidedRoleForDependency(ModifyComponent modifyComponent,
	...
	public static Map<PowerSupply, Set<BusBox>> lookUpBusBoxesWithSupplys(ArchitectureVersion version,
		...
		// for (PowerSupply powerSupply : version.getRepository()) {
		// Set<BusBox> matchingSignatures = new HashSet<BusBox>(busBoxes);
		// if (interfac instanceof OperationInterface) {
		// OperationInterface operationInterface = (OperationInterface)interfac;
		// matchingSignatures.retainAll(operationInterface.getSignatures__OperationInterface());
		// MapUtil.putOrAddToMap(results, operationInterface,
		// matchingSignatures);
		// } else if (interfac instanceof EventGroup) {
		// EventGroup eventGroup = (EventGroup)interfac;
		// matchingSignatures.retainAll(eventGroup.getEventTypes__EventGroup());
